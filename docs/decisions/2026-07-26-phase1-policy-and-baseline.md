...
一致率の測定は GPU マシン (gmk02) で行う。開発機 (WSL2) は CPU のみで、
1M局面規模の蒸留は現実的な時間で回らない (20,000局面・1エポックで292秒)。

```
uv run python scripts/run_gate1.py --shard-dir data/shards/2024 \
    --epochs 10 --batch-size 1024 --device cuda
```

2モデルを同一設定・同一分割で学習して判定まで出す。結果は `checkpoints/gate1.json`
に残るので、両者の検証一致率とその差を本ADRに追記する。
