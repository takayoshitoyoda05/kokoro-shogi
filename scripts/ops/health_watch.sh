...
LOG=checkpoints/e_chain.log; hot=0; lasthb=0; seen=$(grep -cE "Traceback|OOM|CUDA|RuntimeError|FAILED" $LOG)
while true; do
  ...
  if ! grep -qE "\[chain\] ALL DONE|FAILED" $LOG && ! pgrep -f "ops/e_chain.sh" >/dev/null; then echo "HEALTH-ALERT e_chain が終了印なしに消滅"; exit 1; fi
  ...
  now=$(date +%s); if [ $((now-lasthb)) -ge 3600 ]; then stage=$(grep -oE "^\[E[0-9]-[a-z0-9]+\] START" $LOG | tail -1 | cut -d' ' -f1); echo "HEALTH-OK $(date '+%H:%M') GPU ${t}℃ ${m}MiB RAM残${ram}G | $stage"; lasthb=$now; fi
  if grep -qE "\[chain\] ALL DONE|FAILED" $LOG; then echo "HEALTH-END チェーン終了"; exit 0; fi
  sleep 120
done
