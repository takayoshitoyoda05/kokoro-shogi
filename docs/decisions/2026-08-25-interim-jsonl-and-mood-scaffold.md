...
### 5. まだやっていないこと (Phase 3 本体)

- **系列学習**: シャードは棋譜順で game_index を持つので系列は復元できるが、
  イベント特徴 (capture の有無・取られた駒) はシャードに入っていない。
  make_labels に event 列を足すか、dataset 側で連続行の差分から復元するかは
  学習ループ設計 (BPTT長・シャッフル単位=対局) と併せて次のADRで決める
- 学習済みGRUができたら `export_model_jsonl.py` の mood をヒューリスティックから
  差し替え、`--checkpoint` 1つで完全版の納品になる
