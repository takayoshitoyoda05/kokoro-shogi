...
def select_and_mutate(
    cultures: list[Culture], rng: np.random.Generator, *, replaced: int
) -> list[str]:
    """下位 `replaced` 文化を、最良文化の複製+変異で置き換える (PBT)。"""
    ranked = sorted(cultures, key=lambda culture: culture.win_rate, reverse=True)
    best = ranked[0]
    sigma = SIGMA_PBT_RELATIVE * float(best.theta_sp.std().clamp(min=1e-6))
    renewed = []
    for culture in ranked[-replaced:]:
...
def main() -> None:
    ...
    cultures = [
        Culture(
            ...
            tau=float(0.8 * np.exp(rng.normal(0.0, HYPER_JITTER))),
            lambda_g=float(config.loss.lambda_g * np.exp(rng.normal(0.0, HYPER_JITTER))),
        )
    ...
    for generation in range(1, args.generations + 1):
        ...
        renewed = select_and_mutate(cultures, rng, replaced=args.replaced)
        ...
        print(
            f"gen {generation}: WR {entry['win_rates']} / 淘汰 {renewed}"
        ...
        torch.save(
            {
                ...
                "features": saved,
                "cultures": {
                    c.name: {"theta_sp": c.theta_sp, "tau": c.tau, "lambda_g": c.lambda_g}
                    for c in cultures
                },
            },
